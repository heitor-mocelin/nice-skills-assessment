...
import TOPIC_DESCRIPTIONS from "@/data/topic-descriptions.json";
 ...
 *
 * Topic hover-tooltip descriptions live in src/data/topic-descriptions.json
 * (keyed by subdomain id -> topic index) so they can be edited without
 * touching this file. RAW_SUBDOMAINS below only defines the short labels
 * and structural metadata; descriptions are merged in at load time.
 */

const RAW_SUBDOMAINS: { id: string; domainId: string; title: string; workRoles: { code: string; name: string }[]; topics: string[] }[] = [
  ...
  /* ---------------------------- Implementation & Operation (IO) ---------------------------- */
  {
    ...
    workRoles: [
      { code: "IO-WRL-004", name: "Network Operations" },
    ],
    topics: [
  ...
  },
  {
    ...
    workRoles: [
      { code: "IO-WRL-006", name: "Systems Security Analysis" },
    ],
    topics: [
  ...
  },
  {
    ...
    workRoles: [
      { code: "PD-WRL-007", name: "Vulnerability Analysis" },
    ],
    topics: [
  ...
  /* ---------------------------- Investigation (IN) ---------------------------- */
  {
    ...
    workRoles: [
      { code: "IN-WRL-001", name: "Cybercrime Investigation" },
    ],
    topics: [
  ...
  },
  {
    ...
    workRoles: [
      { code: "IN-WRL-002", name: "Digital Evidence Analysis" },
    ],
    topics: [
...
/** Merges RAW_SUBDOMAINS structural data with editable topic descriptions from JSON. */
export const SUBDOMAINS: Subdomain[] = RAW_SUBDOMAINS.map((s) => ({
  id: s.id as Subdomain["id"],
  domainId: s.domainId as Subdomain["domainId"],
  title: s.title,
  workRoles: s.workRoles,
  topics: s.topics.map((label, i) => ({
    label,
    description:
      (TOPIC_DESCRIPTIONS as Record<string, Record<string, string>>)[s.id]?.[String(i)] ?? "",
  })),
}));
